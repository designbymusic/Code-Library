...
// Style for Tumblog Options page
if ( isset( $_GET['page'] ) && $_GET['page'] == 'tumblog') {
...
function woo_load_tumblog_libraries() {
	wp_enqueue_script('newscript', WP_PLUGIN_URL . '/woo-tumblog'.'/functions/js/tumblog-ajax.js', array('jquery', 'jquery-form'));
	wp_enqueue_script('nicedit', WP_PLUGIN_URL . '/woo-tumblog'.'/functions/js/nicEdit.js');
 	wp_enqueue_script('phpjs', WP_PLUGIN_URL . '/woo-tumblog'.'/functions/js/php.js');
 	wp_enqueue_script('datepicker', WP_PLUGIN_URL . '/woo-tumblog'.'/functions/js/ui.datepicker.js',array('jquery'));
}    
...
function woo_load_tumblog_css($hook) {
	if ($hook == 'post.php' OR $hook == 'post-new.php' OR $hook == 'page-new.php' OR $hook == 'page.php' OR ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'woothemes_sbm') ) {
		...
		echo "<link rel='stylesheet' id='colors-tabs' href='".WP_PLUGIN_URL . '/woo-tumblog'."/functions/css/tumblog_admin_styles.css' type='text/css' media='all' />";
		echo "<link rel='stylesheet' id='colors-tabs' href='".WP_PLUGIN_URL . '/woo-tumblog'."/functions/css/jquery-ui-datepicker.css' type='text/css' media='all' />";
	}   
}
...
function woo_tumblog_dashboard_widget_output() {
	...
	if (current_user_can('publish_posts')) {
		...
		?>
		<script type="text/javascript" src="<?php echo WP_PLUGIN_URL . '/woo-tumblog'; ?>/functions/js/ajaxupload.js"></script>
		...
		jQuery(document).ready(function(){
			...
			jQuery('.date-picker').each(function (){
				jQuery('#' + jQuery(this).attr('id')).datepicker({showOn: 'button', buttonImage: '<?php echo WP_PLUGIN_URL . '/woo-tumblog';?>/functions/images/calendar.gif', buttonImageOnly: true});
			});
			...
			jQuery('#articles-menu-button').click(function ()
			{
				...
				if (nicEditors.findEditor('test-content') == undefined) {
					myNicEditor = new nicEditor({ buttonList : ['bold','italic','underline','ol','ul','left','center','right','justify','link','unlink','strikeThrough','xhtml','image'], iconsPath : '<?php echo WP_PLUGIN_URL . '/woo-tumblog'; ?>/functions/images/nicEditorIcons.gif'}).panelInstance('test-content');
				} else {
	...
	<div id="tumblog-post">
		...
		<img id="ajax-loader" src="<?php echo WP_PLUGIN_URL . '/woo-tumblog'; ?>/functions/images/ajax-loader.gif" />
